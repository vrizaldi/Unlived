...
//	public static Animation shadowAnim;
	...
	public static BitmapFont fontUI; // default font
	public static BitmapFont fontCam; 
...
// initialise and dispose resources ---------------------------------------------------------------------------
	public static void init() {
		...
		doorVSprite.setSize(1 + Constants.ROOMS_INTERVAL, 
				1 + Constants.CHAR_HEIGHT + 1);
		
		doorHSprite = new Sprite(mapImgs.findRegion("door"));
		doorHSprite.setSize(Constants.CHAR_WIDTH + 1, 
				1 + Constants.ROOMS_INTERVAL);
...
/*		shadowAnim = new Animation(Constants.ANIMATION_SHADOW_DURATION,
		...
		FreeTypeFontParameter paramUI = new FreeTypeFontParameter();
		paramUI.size = 10;
		paramUI.color = Color.RED;
		fontUI = new FreeTypeFontGenerator(Gdx.files.internal("PressStart2P.ttf")) 
			.generateFont(paramUI);

		FreeTypeFontParameter paramCam = new FreeTypeFontParameter();
		paramCam.size = 10;
		paramCam.color = Color.RED;
		fontCam = new FreeTypeFontGenerator(Gdx.files.internal("PressStart2P.ttf")) 
			.generateFont(paramCam);
