...
/*			// draw the shadow
			...
			texts.setText(Assets.fontCam, String.format("%.2f", c.atts.getMana()));
			Assets.fontCam.draw(batch, texts, 
					(c.x + ((Constants.CHAR_WIDTH - texts.width) / 2)),
					c.y + Constants.CHAR_HEIGHT,
		...
		texts.setText(Assets.fontUI, String.format("%.2f", data.getKillPerDeath()) 
			+ " kills/death");
		Assets.fontUI.draw(batch, texts, ui.viewportWidth - texts.width,
			ui.viewportHeight - texts.height);

		// mainChar mana on top left screen
		texts.setText(Assets.fontUI, "Mana: " 
				+ String.format("%.2f", data.getMainChar().atts.getMana()));
		Assets.fontUI.draw(batch, texts, 0,
