...
// render the menu -------------------------------------------------------------------------------------------
	...
	private void renderOption(final int i) {
		...
		texts.setText(Assets.ins.font, 
				Assets.ins.labels.get(data.cOptions[i]));

		final float OPT_ORIGIN_Y = texts.height * (data.cOptions.length - 1)
					+ gapY * (data.cOptions.length - 2) - textOffsetY * 2;
		final float textY = OPT_ORIGIN_Y - (texts.height * (i + 1))
					- (gapY * i);

		updatePointer();
		if(i == data.pointer) {
			renderPointer(textY, texts.width, texts.height);
		}
		...
		if(data.cOptions == Assets.ins.optionsSetting) {
			renderSetting(i, textY, texts.height);
		}
	}
