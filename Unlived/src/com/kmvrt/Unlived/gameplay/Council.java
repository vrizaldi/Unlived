...
//					Gdx.app.debug(TAG, "mood = " + mood);
	...
	private void creepsAttack() {
		...
		for(GameChar creep : data.chars) {
			if(creep.getID() != Constants.CHAR_MAIN) {
				...
				if(intersectRectangles(rec1, rec2, inter)
	...
	private boolean intersectRectangles(Rectangle rec1, Rectangle rec2, 
			Rectangle inter) {
		
		if (rec1.overlaps(rec2)) {
			inter.x = Math.max(rec1.x, rec2.x);
		    inter.width = Math.min(rec1.x + rec1.width, rec2.x + rec2.width) 
		    		- inter.x;
		    inter.y = Math.max(rec1.y, rec2.y);
		    inter.height = Math.min(rec1.y + rec1.height, 
		    		rec2.y + rec2.height) - inter.y;
		    return true;
		}
		return false;
	}
